...
function parseMovies(text) {
  ...
  for (const line of lines) {
    ...
    const title = parts[0], url = parts[1], image = parts[2] || "";
...
function parseSeriesEpisodes(text) {
  const lines = text.split(/\r?\n/).map(l => l.trim()).filter(Boolean);
  const eps = [];
  for (const line of lines) {
    const parts = line.split("|").map(p => p.trim()).filter(Boolean);
...
exports.handler = async (event) => {
  try {
    ...
    // Decide which file to search based on prefix
    if (id.startsWith("m_")) {
    ...
    if (id.startsWith("e_")) {
      ...
      const eps = parseSeriesEpisodes(raw);
